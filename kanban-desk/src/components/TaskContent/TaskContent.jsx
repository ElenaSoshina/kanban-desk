...
const TaskContent = () => {
    ...
    const [description, setDescription] = useState(task && task.description.length > 0 ? task.description : 'This task no description')

    const handleFocus = () => {
        if (description === 'This task no description') {
            setDescription('')
        }
    }

    const handleClick = () => {
        const newTaskList = tasksList.map(t => {
            if (t.id === task.id) {
                t.description = description === 'This task no description' ? '' : description
            }
            return t
        })
